...
def test_load_settings_success(tmp_path: Path) -> None:
    ...
    assert settings.answer_generation is not None
    assert settings.answer_generation.enabled is False
    assert settings.answer_generation.prompt_path == "./config/prompts/answer_generation.txt"
    assert settings.answer_generation.max_context_chars == 6000


def test_load_answer_generation_settings(tmp_path: Path) -> None:
    config = """
    llm:
      provider: openai
      model: gpt-4o-mini
    ...
    answer_generation:
      enabled: true
      prompt_path: ./config/prompts/custom_answer.txt
    ...
    assert settings.answer_generation is not None
    assert settings.answer_generation.enabled is True
    assert settings.answer_generation.prompt_path == "./config/prompts/custom_answer.txt"
    assert settings.answer_generation.max_context_chars == 3000
